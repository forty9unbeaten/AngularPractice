...
export class ShoppingListService {
  ...
  private ingredientsURL: string =
    'https://angular-practice-8c0db-default-rtdb.firebaseio.com/ingredients';
  ...
  public getIngredientList = (): Observable<Ingredient[]> => {
    return this.http.get(`${this.ingredientsURL}.json`).pipe(
  ...
  private sendIngredientsToServer = (): void => {
    this.http.put(`${this.ingredientsURL}.json`, this.ingredients).subscribe(
      (res: Ingredient[]) => {
        if (res) {
