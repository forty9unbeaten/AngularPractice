...
export class RecipesService {
  ...
  private recipesURL: string =
    'https://angular-practice-8c0db-default-rtdb.firebaseio.com/recipes';
  ...
  public getSelectedRecipe = (id: string): Observable<Recipe> => {
  ...
  public addRecipe = (recipe: Recipe): void => {
    this.http.post(`${this.recipesURL}.json`, recipe).subscribe(
      (res: { name: string }) => {
        recipe.id = res.name;
  ...
  public deleteRecipe = (recipe: Recipe): void => {
    this.http.delete(`${this.recipesURL}/${recipe.id}.json`).subscribe(
      (res) => {
  ...
  public changeRecipe = (recipe: Recipe): void => {
    this.http.patch(`${this.recipesURL}/${recipe.id}.json`, recipe).subscribe(
      (recipe: Recipe) => {
